...
def download_assets():
    ...
    os.system('curl -L "https://drive.google.com/uc?export=download&id=1gJ-g7EWreZu38DlaZltLDylPIBDHLSxX" -o assets.tar')
...
def update_metadata(next_module_name):
    ...
    for line in fileinput.input(files ='__CODEISGOD__/metadata.json', inplace=1):
...
def push_to_repo(next_module_name):
    ...
    repo = git.Repo('.')
    repo.index.add(['__CODEISGOD__/metadata.json'])
